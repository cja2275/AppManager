...
<form
	...
	<div>
		下载次数: <input name="downloads" type="text"
			value="${app_Info.downloads}">
	</div>
	<div>
	...
	<div>
		APP状态: <select id="status" name="status">
			<c:forEach var="statu" items="${status }">
				<option value="${statu.valueId}"
					${app_Info.status==statu.valueId?'selected':''}>${statu.valueName} </option>
			</c:forEach>
		</select>
	</div>
	<div>
		应用简介:
		<textarea name="appInfo" rows="4" cols="">${app_Info.appInfo }</textarea>
	</div>
	<div>
		LOGO图片 : <input type="file">
	</div>
	<div>
		<input type="submit" value="提交">
	</div>
	<input type="text" name="id" id="id" value="${app_Info.id }"/>
